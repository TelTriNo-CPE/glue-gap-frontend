            ...
            <section>
              ...
              <dl className="flex flex-col gap-2">
                <div className="flex justify-between text-sm">
                  ...
                  <dd className="font-medium text-gray-900">{result.gap_count}</dd>
                </div>
                <div className="flex justify-between text-sm">
                ...
                <ul className="flex flex-col gap-1">
                  {result.gaps.map((gap, index) => (
                    <li key={index}
                      ...
                      <button
                        onClick={() => onToggleGap(index)}
                        title={hiddenGapIndices.has(index) ? 'Show overlay' : 'Hide overlay'}
                        className="shrink-0 text-gray-400 hover:text-gray-700"
                        aria-label={`Toggle visibility for gap ${index + 1}`}
                      >
                        {hiddenGapIndices.has(index) ? <EyeOffIcon /> : <EyeIcon />}
                      </button>
                      <span className="flex-1 text-gray-700">Gap {index + 1}</span>
