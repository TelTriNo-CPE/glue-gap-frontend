...
const btnClass =
  'w-10 h-10 rounded-lg text-gray-400 hover:text-white hover:bg-gray-700 transition-colors flex items-center justify-center disabled:opacity-40 disabled:cursor-not-allowed';
...
export default function Toolbar({ stem, fileKey, hasResult, analyzing, onAnalyze, onReset }: Props) {
  ...
  return (
    <aside className="w-16 bg-gray-900 flex flex-col items-center py-4 gap-4 shrink-0">
      {/* Upload Another */}
      <button onClick={onReset} title="Upload another file" className={btnClass}>
      ...
      {/* Change to Greyscale / analyzing state */}
      <button
        onClick={onAnalyze}
        disabled={hasResult || analyzing}
        title={hasResult ? 'Analysis complete' : 'Change to Greyscale & detect gaps'}
        className={btnClass}
      >
        {analyzing ? <Spinner /> : hasResult ? (
          ...
          // Greyscale icon (half-filled circle)
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            <path strokeLinecap="round" strokeLinejoin="round"
